...
This bot was made by molten_is_glitched and baconmaxyy.
...
class CatchModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.correct_name.lower() == "ball 1":
            ...
            self.stats['hp'] = random.randint(100, 125)
            self.stats['attack'] = random.randint(85, 100)

        user_owns_ball = check_if_user_owns_ball(interaction.user.id, self.correct_name)
        shiny_status = "Yes" if random.randint(1, 200) == 1 else "No"
...
async def spawn_countryball(channel):
    ...
    view = discord.ui.View()
    catch_button = discord.ui.Button(label="Catch this UniversalBall!", style=discord.ButtonStyle.primary)
